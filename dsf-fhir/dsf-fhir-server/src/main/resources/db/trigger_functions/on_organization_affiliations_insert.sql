...
DECLARE
	...
	delete_count INT;
BEGIN
	PERFORM on_resources_insert(NEW.organization_affiliation_id, NEW.organization_affiliation);
	...
	IF (NEW.organization_affiliation->>'active' = 'true') THEN
		...
		IF (consortium_identifier IS NOT NULL AND member_organization_id IS NOT NULL) THEN
			...
			INSERT INTO read_access 			
				SELECT DISTINCT r.id, 'ROLE', member_organization_id, NEW.organization_affiliation_id
				FROM (
					SELECT 
