CREATE OR REPLACE FUNCTION on_resources_insert(new_resource_id uuid, new_resource jsonb) RETURNS void AS $$
DECLARE
...
BEGIN
	-- delete entries of binaries that use this new/updated resource as security context 
	DELETE FROM read_access WHERE resource_id IN (
		SELECT binary_id FROM current_binaries 
		WHERE new_resource_id = (regexp_match(binary_json->'securityContext'->>'reference', reference_regex))[5]::uuid
	);
	...
	-- delete existing entries for this new/updated resource
	DELETE FROM read_access WHERE resource_id = new_resource_id;
	...
	IF (new_resource->'meta'->'tag' @> '[{"system":"http://highmed.org/fhir/CodeSystem/read-access-tag","code":"ALL"}]'::jsonb) THEN
		...
		VALUES(new_resource_id, 'ALL', NULL, NULL);
	...
	IF (new_resource->'meta'->'tag' @> '[{"system":"http://highmed.org/fhir/CodeSystem/read-access-tag","code":"LOCAL"}]'::jsonb) THEN
		INSERT INTO read_access
		VALUES(new_resource_id, 'LOCAL', NULL, NULL);
	...
	IF (new_resource->'meta'->'tag' @> '[{"system":"http://highmed.org/fhir/CodeSystem/read-access-tag","code":"ORGANIZATION"}]'::jsonb) THEN
		INSERT INTO read_access 			
		SELECT new_resource_id, 'ORGANIZATION', organization_id, NULL FROM (
	...
	IF (new_resource->'meta'->'tag' @> '[{"system":"http://highmed.org/fhir/CodeSystem/read-access-tag","code":"ROLE"}]'::jsonb) THEN
		INSERT INTO read_access 			
		SELECT new_resource_id, 'ROLE', member_organization_id, organization_affiliation_id FROM (
	...
	-- add entries for binaries that use this new/updated resource as security context
	INSERT INTO read_access
