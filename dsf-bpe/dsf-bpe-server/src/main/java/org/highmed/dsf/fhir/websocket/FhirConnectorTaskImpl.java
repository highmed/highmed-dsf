...
public class FhirConnectorTaskImpl implements InitializingBean, FhirConnector
{
	private static final Logger logger = LoggerFactory.getLogger(FhirConnectorTaskImpl.class);
	...
	public FhirConnectorTaskImpl(FhirWebsocketClientProvider clientProvider, TaskHandler handler,
	...
	private Subscription loadExistingResources(Subscription subscription)
	{
		...
		ExistingTaskLoader existingTaskLoader = new ExistingTaskLoader(lastEventTimeIo, handler, webserviceClient);
		Map<String, List<String>> subscriptionCriteria = parse(subscription.getCriteria(), "Task");
		existingTaskLoader.readExistingResources(subscriptionCriteria);
	...
	{
		FhirWebserviceClient webserviceClient = clientProvider.getLocalWebserviceClient();
		PingEventTaskHandler pingHandler = new PingEventTaskHandler(
				new ExistingTaskLoader(lastEventTimeIo, handler, webserviceClient));
	...
	{
		EventTaskHandler eventHandler = new EventTaskHandler(lastEventTimeIo, handler);
