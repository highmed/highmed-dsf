...
public class StoreResult extends AbstractServiceDelegate implements InitializingBean
{
	...
	protected void doExecute(DelegateExecution execution) throws Exception
	{
		...
		FeasibilityQueryResults results = (FeasibilityQueryResults) execution.getVariable(Constants.VARIABLE_QUERY_RESULTS);

		addOutputs(task, results);
		execution.setVariable(Constants.VARIABLE_TASK, task);
	}
	...
	private void addOutput(Task task, FeasibilityQueryResult result)
	{
		...
		{
			Task.TaskOutputComponent output = getTaskHelper().createOutputUnsignedInt(Constants.CODESYSTEM_HIGHMED_FEASIBILITY,
		...
		{
			Task.TaskOutputComponent output = getTaskHelper().createOutput(Constants.CODESYSTEM_HIGHMED_FEASIBILITY,
					Constants.CODESYSTEM_HIGHMED_FEASIBILITY_VALUE_SINGLE_MEDIC_RESULT_REFERENCE, new Reference(result.getResultSetUrl()));
			output.addExtension(createCohortIdExtension(result.getCohortId()));
			task.addOutput(output);
		}
		else
