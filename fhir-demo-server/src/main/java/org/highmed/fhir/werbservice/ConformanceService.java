...
public class ConformanceService
{
	...
	public Response getMetadata(@QueryParam("mode") String mode)
	{
		...
		CapabilityStatementRestResourceComponent resource1 = rest.addResource();
		resource1.setType(Patient.class.getAnnotation(ResourceDef.class).name());
		resource1.setProfile(Patient.class.getAnnotation(ResourceDef.class).profile());
		resource1.addInteraction().setCode(TypeRestfulInteraction.CREATE);
		resource1.addInteraction().setCode(TypeRestfulInteraction.READ);
