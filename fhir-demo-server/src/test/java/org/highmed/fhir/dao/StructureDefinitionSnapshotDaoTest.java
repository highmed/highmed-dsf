		...
		extends AbstractDomainResourceDaoTest<StructureDefinition, StructureDefinitionSnapshotDao>
{
	...
	{
		return new StructureDefinitionSnapshotDao(dataSource, fhirContext, new SnapshotInfoConverter(objectMapper));
	}
...
//						"SELECT structure_definition_snapshot_info FROM structure_definition_snapshots WHERE structure_definition_snapshot_id = ?"))
//		{
...
//		try (Connection connection = database.getDataSource().getConnection();
//				PreparedStatement statement = connection.prepareStatement(
//						"SELECT count(*) FROM structure_definition_snapshots WHERE structure_definition_snapshot_info->'dependencies'->'profiles' ?? ?"))
//		{
